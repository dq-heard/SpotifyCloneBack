package app.dq_heard.clone_backend.catalogcontext.presentation;

import app.dq_heard.clone_backend.catalogcontext.application.SongService;
...
import app.dq_heard.clone_backend.infrastructure.service.dto.StatusNotification;
...
public class SongResource {
  ...
  public ResponseEntity<ReadSongInfoDTO> add(@RequestPart(name = "cover") MultipartFile cover,
    ...
    saveSongDTO = new SaveSongDTO(saveSongDTO.title(), saveSongDTO.artist(),
  ...
  public ResponseEntity<FavoriteSongDTO> addOrRemoveFromFavorites(@Valid @RequestBody FavoriteSongDTO favoriteSongDTO) {
    ReadUserDTO userFromAuthentication = userService.getAuthenticatedUserFromSecurityContext();
    State<FavoriteSongDTO, String> favoriteSongResponse = songService.addOrRemoveFromFavorites(favoriteSongDTO, userFromAuthentication.email());
